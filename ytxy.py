...
def handle_existing_file(title, file_format):
    new_filename = title  # Algne väärtus
    ...
    def on_overwrite():
        nonlocal new_filename
        new_filename = title  # Kasutab olemasolevat nime
        dialog.destroy()

    def on_cancel():
        nonlocal new_filename
        new_filename = None  # Katkestab allalaadimise
        dialog.destroy()
...
def get_unique_filename(url, file_format):
    ...
    if os.path.exists(file_path):
        ...
        if new_title is None:
            return None  # Kui kasutaja katkestab, siis lõpetame
...
def download_video():
    ...
    if output_filename is None:
        status_label.config(text="Allalaadimine tühistatud!", fg="orange")
        return
...
resolution_menu = tk.OptionMenu(root, resolution_var, 'best', '720p', '1080p', '1440p', '2160p', 'worst', 'MP3 (audio only)')
